...
for nTrees = numTreesList
    for leafSize = leafSizeList
        ...
        fprintf('Testing GB: %d trees, MinLeafSize=%d\n', nTrees, leafSize);
        tic;
        for fold = 1:cv_kfold.NumTestSets
            ...
            % Train model
            t_tree = templateTree('MinLeafSize', leafSize, 'Surrogate', 'on');
...
model_filename = fullfile(output_folder, ...
    sprintf('best_gradient_boosting_%dtree_leaf%d.mat', best_params.NumTrees, best_params.MinLeafSize));

save(model_filename, 'best_gb_model', 'best_params', ...
    'train_rmse', 'train_mae', 'train_r2', ...
    'test_rmse', 'test_mae', 'test_r2');
